!function(factory) {
  ...
  Windex.prototype = {
    ...
    negotiate: function(request, deferred) {
      var contentType = request.getResponseHeader('Content-Type') ? request.getResponseHeader('Content-Type').split(';')[0] : 'text/plain';

        if (contentType && typeof this.parsers[contentType] === 'function') {
            try {
                if ((request.status < 200 || request.status > 299) && request.status !== 304) {
                    deferred.reject(this.parsers[contentType](request.responseText));
                } else {
